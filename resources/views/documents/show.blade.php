    ...
    <div class="w-full max-w-container-max mx-auto px-gutter py-xl">
        ...
        <section class="mb-xxl flex flex-col md:flex-row md:items-end justify-between gap-md">
            <div>
                ...
                <p class="font-body-lg text-body-lg text-on-surface-variant mt-sm">Reviewing <span class="font-semibold text-on-surface">{{ $document->original_name }}</span> for compliance.</p>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-lg">
                    <div class="bg-surface-container-lowest border border-outline-variant p-lg rounded-xl flex flex-col gap-sm">
                        ...
                        <div class="flex items-center gap-md">
                            ...
                            <span class="font-headline-sm text-headline-sm uppercase">{{ $document->file_type }} Document</span>
                        </div>
                    </div>
                    <div class="bg-surface-container-lowest border border-outline-variant p-lg rounded-xl flex flex-col gap-sm">
                        ...
                        <div class="flex items-center gap-md">
                            ...
                            <span class="font-headline-sm text-headline-sm">{{ $analysis->ai_confidence ?? 'N/A' }}%</span>
                        </div>
                    </div>
                </div>
    ...
    <p class="text-body-md text-on-surface-variant mt-md leading-relaxed whitespace-pre-line">
        {{ $analysis->summary }}
                        ...
                        <div class="mt-xl pt-lg border-t border-outline-variant grid grid-cols-3 gap-md">
                            ...
                            <div>
                                ...
                                <p class="font-body-md text-body-md font-semibold text-danger text-red-600">{{ $analysis->critical_issues }} Issues</p>
                            </div>
                            <div>
                                <p class="font-label-md text-label-md text-on-surface-variant mb-xs">Status</p>
                                <p class="font-body-md text-body-md font-semibold text-green-600 uppercase">{{ $document->status }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-outline-variant p-lg rounded-xl shadow-sm flex flex-col items-center text-center hover:translate-y-[-2px] transition-transform duration-200 ease-out">
                    ...
                    <div class="relative w-32 h-32 flex items-center justify-center mb-md">
                        <svg class="absolute inset-0 w-full h-full" viewbox="0 0 100 100">
                            ...
                            <circle class="{{ $analysis->risk_score > 50 ? 'text-red-500' : 'text-green-500' }}" cx="50" cy="50" fill="transparent" r="42" stroke="currentColor" stroke-dasharray="264" stroke-dashoffset="{{ 264 - (264 * ($analysis->risk_score ?? 0)) / 100 }}" stroke-linecap="round" stroke-width="8"></circle>
                        </svg>
                        <div class="flex flex-col">
                            <span class="font-headline-xl text-headline-xl {{ $analysis->risk_score > 50 ? 'text-red-600' : 'text-green-600' }}">{{ $analysis->risk_score ?? 0 }}</span>
                            <span class="font-label-md text-label-md text-on-surface-variant -mt-1">/100</span>
                        </div>
                    </div>
                    <div class="{{ $analysis->risk_score > 50 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }} px-md py-sm rounded-full font-bold flex items-center gap-xs">
                        <span class="material-symbols-outlined text-[20px]" style="font-variation-settings: 'FILL' 1;">shield</span>
                        Risk Factor Mode
                    </div>
                    <p class="font-body-sm text-body-sm text-on-surface-variant mt-md">This score models potential contract liabilities identified by AI reasoning.</p>
                </div>
                ...
                <div class="bg-surface-container-low border border-outline-variant p-lg rounded-xl">
                    ...
                    <div class="space-y-sm max-h-80 overflow-y-auto scrollbar-none text-right" dir="rtl">
                        @forelse($analysis->clauses_analysis as $item)
                            <div class="bg-white p-sm rounded-lg flex flex-col gap-xs border border-outline-variant/30 hover:border-primary/40 transition-colors">
                                <div class="flex justify-between items-center w-full">
                                    <span class="font-body-md text-body-md text-primary font-semibold">{{ $item['clause'] ?? 'بند فرعي' }}</span>
                                    <span class="material-symbols-outlined text-green-600 text-[18px]">verified</span>
                                </div>
                                <p class="text-xs text-on-surface-variant leading-relaxed">{{ $item['analysis'] ?? '' }}</p>
                            </div>
                        @empty
                            <p class="text-xs text-on-surface-variant text-center py-md">No specific clauses identified.</p>
                        @endforelse
                    </div>
        ...
        <section class="mt-xxl">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-lg">
                <div class="lg:col-span-2 bg-white border border-outline-variant rounded-xl p-xl h-96 overflow-y-auto relative text-right" dir="rtl">
